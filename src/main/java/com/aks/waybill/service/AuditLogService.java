...
public final class AuditLogService {
    ...
    public record AuditRecord(long id, String userName, String action, String entityType, Long entityId, String details, LocalDateTime createdAt) {}
    public static void log(String action, String entityType, Long entityId, String details) {
        ...
        try (Connection c=Database.getConnection(); PreparedStatement p=c.prepareStatement("INSERT INTO audit_log(user_id,action,entity_type,entity_id,details,created_at) VALUES(?,?,?,?,?,?)")) {
            if(userId==null)p.setNull(1,Types.INTEGER);else p.setLong(1,userId); p.setString(2,action); p.setString(3,entityType); if(entityId==null)p.setNull(4,Types.INTEGER);else p.setLong(4,entityId); p.setString(5,details); p.setString(6,LocalDateTime.now().toString()); p.executeUpdate();
    ...
    public static Page findPage(String search,int page,int pageSize){
        ...
        String where=term.isBlank()?"":" WHERE COALESCE(u.username,'') LIKE ? COLLATE NOCASE OR a.action LIKE ? COLLATE NOCASE OR COALESCE(a.entity_type,'') LIKE ? COLLATE NOCASE OR COALESCE(a.details,'') LIKE ? COLLATE NOCASE "; String like="%"+term+"%";
        try(Connection c=Database.getConnection()){
            long total; try(PreparedStatement p=c.prepareStatement("SELECT COUNT(*) FROM audit_log a LEFT JOIN app_user u ON u.id=a.user_id"+where)){if(!term.isBlank()){for(int i=1;i<=4;i++)p.setString(i,like);}try(ResultSet r=p.executeQuery()){r.next();total=r.getLong(1);}}
            int pages=(int)Math.max(1,(total+size-1)/size), normalized=Math.min(requested,pages-1); List<AuditRecord> rows=new ArrayList<>();
            String sql="SELECT a.id,COALESCE(u.username,'System'),a.action,COALESCE(a.entity_type,''),a.entity_id,COALESCE(a.details,''),a.created_at FROM audit_log a LEFT JOIN app_user u ON u.id=a.user_id"+where+" ORDER BY a.id DESC LIMIT ? OFFSET ?";
            try(PreparedStatement p=c.prepareStatement(sql)){int i=1;if(!term.isBlank()){for(int n=0;n<4;n++)p.setString(i++,like);}p.setInt(i++,size);p.setInt(i,normalized*size);try(ResultSet r=p.executeQuery()){while(r.next())rows.add(new AuditRecord(r.getLong(1),r.getString(2),r.getString(3),r.getString(4),readLong(r, 5), r.getString(6), parseDateTime(r.getString(7))));}}

...
public class AuthService {
    public boolean hasUsers() {
        ...
        catch(SQLException e){ WaspLogger.error("Unable to check users", e); throw new IllegalStateException("Unable to check users",e); }
    }
    public void createInitialAdmin(String username,String displayName,String userCode,String password) {
        ...
        try(Connection c=Database.getConnection(); PreparedStatement p=c.prepareStatement("INSERT INTO app_user(username,password_hash,display_name,user_code,role,created_at,updated_at) VALUES(?,?,?,?,?,?,?)")){
            ...
            WaspLogger.info("Initial administrator created. username=" + username.trim());
        }catch(SQLException e){ WaspLogger.error("Unable to create administrator", e); throw new IllegalStateException("Unable to create administrator",e);}
    }
    public Optional<UserRecord> authenticate(String username,String password) {
        ...
        try(Connection c=Database.getConnection(); PreparedStatement p=c.prepareStatement(sql)){
            ...
            try(ResultSet r=p.executeQuery()){
                if(!r.next() || r.getInt("enabled")!=1) { WaspLogger.warning("Login failed. username=" + username.trim() + ", reason=unknown-or-disabled-user"); return Optional.empty(); }
                String lock=r.getString("locked_until"); if(lock!=null && Instant.parse(lock).isAfter(Instant.now())) { WaspLogger.warning("Login blocked by temporary lockout. username=" + username.trim()); return Optional.empty(); }
                long id=r.getLong("id");
                if(!PasswordService.matches(password,r.getString("password_hash"))){ registerFailedAttempt(id,r.getInt("failed_login_attempts")); WaspLogger.warning("Login failed. username=" + username.trim() + ", reason=invalid-password"); return Optional.empty(); }
                resetLoginState(id);
                WaspLogger.info("User authenticated successfully. username=" + r.getString("username") + ", userId=" + id);
                return Optional.of(new UserRecord(id,r.getString("username"),r.getString("display_name"),r.getString("user_code"),r.getString("role"),r.getInt("must_change_password") == 1));
            }
        }catch(SQLException e){ WaspLogger.error("Unable to authenticate user", e); throw new IllegalStateException("Unable to authenticate user",e);}
    }

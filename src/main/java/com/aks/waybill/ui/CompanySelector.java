...
/**
 * Company selector backed by the single company master.
 *
 * It provides an explicit saved-company drop-down plus a free-text company
 * field with non-destructive autocomplete suggestions. Typing never commits
 * a suggestion automatically; an existing company is selected only by using
 * the drop-down, clicking a suggestion, or pressing Enter.
 */
public final class CompanySelector extends VBox {
    ...
    public CompanySelector(Consumer<CompanyService.CompanyRecord> selectionListener,
        ...
        savedCompany.setPromptText("— Select saved company —");
        savedCompany.setMaxWidth(Double.MAX_VALUE);
        savedCompany.getStyleClass().add("settings-field");
        ...
        HBox inputRow = new HBox(8, editor, newButton);
        ...
        editor.textProperty().addListener((obs, oldValue, newValue) -> {
            if (!internalChange) {
                ...
                savedCompany.getSelectionModel().clearSelection();
        ...
        savedCompany.setOnAction(event -> {
            ...
            CompanyService.CompanyRecord record = findByCompanyName(name);
        ...
        editor.setOnAction(event -> {
            ...
            if (record == null && !suggestionList.getSelectionModel().isEmpty()) {
                record = findByCompanyName(suggestionList.getSelectionModel().getSelectedItem());
            }
            if (record != null) selectRecord(record);
        ...
        suggestionList.setOnMouseClicked(event -> {
            if (event.getClickCount() == 1) selectHighlightedSuggestion();
        });
        suggestionList.setOnKeyPressed(event -> {
            switch (event.getCode()) {
                case ENTER -> { selectHighlightedSuggestion(); event.consume(); }
                case ESCAPE -> { hideSuggestions(); editor.requestFocus(); event.consume(); }
                default -> { }
            }
        });
    ...
    public void refreshCompanies() {
        ...
        if (previousName != null) {
            CompanyService.CompanyRecord record = findByCompanyName(previousName);
            if (record != null) selectRecord(record);
        } else {
    ...
    public void selectCompany(long companyId) {
        ...
        if (record == null) { clearSelection(); return; }
        selectRecord(record);
    }
    ...
    public void selectCompanyByName(String companyName) {
        ...
        if (record == null) record = CompanyService.findByName(companyName.trim(), false);
        if (record == null) setTypedCompanyName(companyName); else selectRecord(record);
    }
    ...
    public void setReadOnly(boolean readOnly) {
        savedCompany.setDisable(readOnly);
    ...
    private void selectRecord(CompanyService.CompanyRecord record) {
        ...
        if (!savedCompany.getItems().contains(record.companyName())) savedCompany.getItems().add(record.companyName());
        savedCompany.getSelectionModel().select(record.companyName());
        internalChange = false;
        hideSuggestions();
        notifySelection();
    }
    ...
    private void filterAndShowSuggestions(String text) {
        ...
        suggestionList.getItems().setAll(filtered.stream().map(CompanyService.CompanyRecord::companyName).toList());
        if (getCompanyName().isBlank() || filtered.isEmpty() || !editor.isFocused()) { hideSuggestions(); return; }
        suggestionList.getSelectionModel().clearSelection();
        showSuggestions();
    }

    private void showSuggestions() {
        if (suggestionList.getItems().isEmpty() || !editor.isFocused() || editor.getScene() == null || editor.getScene().getWindow() == null) return;
        double width = Math.max(editor.getWidth(), 300);
        suggestionList.setPrefWidth(width); suggestionList.setMinWidth(width); suggestionList.setMaxWidth(width);
        var bounds = editor.localToScreen(editor.getBoundsInLocal());
        if (bounds != null) suggestionPopup.show(editor, bounds.getMinX(), bounds.getMaxY());
    }
    ...
    private List<CompanyService.CompanyRecord> filterRecords(String text) {
        ...
        return allCompanies.stream().filter(c -> term.isBlank()
                ...
                || safe(c.emailAddress()).toLowerCase().contains(term)).toList();
    }
    ...
    private CompanyService.CompanyRecord findByCompanyName(String companyName) {
        ...
        return allCompanies.stream().filter(c -> c.companyName().equalsIgnoreCase(companyName.trim())).findFirst()
    ...
    private void createNewCompany() {
        ...
        dialog.setTitle("New Company"); dialog.setHeaderText("Add a company to the master data"); dialog.initModality(Modality.APPLICATION_MODAL);
        ...
        VBox content = new VBox(10, labeled("Company Name", name), labeled("Contact Person", contact), labeled("Address", address), labeled("Phone Number", phone), labeled("Email Address", email));
        content.setPrefWidth(480); dialog.getDialogPane().setContent(content);
        ...
        dialog.setResultConverter(button -> {
            ...
            try { return CompanyService.create(name.getText(), contact.getText(), address.getText(), phone.getText(), email.getText()); }
            catch (RuntimeException ex) { showError(dialog, ex.getMessage()); return null; }
        });
        dialog.showAndWait().ifPresent(record -> { allCompanies = CompanyService.findActiveCompanies(); savedCompany.getItems().setAll(allCompanies.stream().map(CompanyService.CompanyRecord::companyName).toList()); selectRecord(record); companyMasterChangedListener.run(); });
    }

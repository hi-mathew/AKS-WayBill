...
public final class CompaniesView extends AppView {
    ...
    private static final class CompanyMasterTab extends VBox {
        ...
        private void openCompanyDialog(CompanyService.CompanyRecord existing) {
            ...
            dialog.setResultConverter(button -> { if (button != save) return null; try { return existing == null ? CompanyService.create(type, name.getText(), contact.getText(), address.getText(), phone.getText(), email.getText()) : CompanyService.update(type, existing.id(), name.getText(), contact.getText(), address.getText(), phone.getText(), email.getText(), active.isSelected()); } catch (RuntimeException ex) { showError(ex.getMessage()); return null; } });
